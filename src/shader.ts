    ...
    init = (
        ...
        if (this.fragment) {
            ...
            this.fragment.bindGroupLayout = this.device.createBindGroupLayout({
                entries: this.createBindGroupFromEntries(this.fragment, 'fragment')
            });
...
fn frag_main(
    ...
    updateGraphicsPipeline = (
        ...
        let pipeline = this.createRenderPipelineDescriptor(nVertexBuffers, swapChainFormat);
...
export class ShaderContext {
    setBindGroupLayout(entries: void) {
        throw new Error("Method not implemented.");
    }
    createBindGroupEntries(_textures: { [key: string]: { data: Uint8Array; width: number; height: number; bytesPerRow?: number; label?: string; format?: string; usage?: any; samplerSettings?: any; } | ImageBitmap; }) {
        throw new Error("Method not implemented.");
    }
